...
if (isset($_POST['updateInfoBtn'])) {
    $originalUsername = $_POST['oUsername'];
    ...
    $updateImageTable = mysqli_query($mysqli, "UPDATE userimages SET email = '".$cEmail."', username='".$cUsername."' WHERE email = '$email' AND username = '$originalUsername'");
    $updateCommentsTable = mysqli_query($mysqli, "UPDATE image_comments SET username='".$cUsername."' WHERE username='$originalUsername'");
}
...
<head>
    ...
    <link href="style/editprofileInfo2.css" rel="stylesheet" type="text/css">
        ...
        if ($email && $pass) {
            ...
            if ($row = mysqli_fetch_array($res)) {
                ...
                echo ' <div class="container">
                    <h3 style="text-align: center; text-decoration:underline">Select a box to change your information</h3>
                    <div id="topBox">
                        ...
                        <form action="editprofile.php" method="post">
                            <div class="row">
                                <div class="col-5 details">
                                    ...
                                    <span data-value="fname" name="fname">'.$row['fname'].'</span><br>
                                    <h3 class="btn">Edit</h3>
                                </div>
                                <div class="col-5 details">
                                    <label for="fname">Surname:</label>
                                    <span data-value="lname" name="lname">'.$row['lname'].'</span><br>
                                    <h3 class="btn">Edit</h3>
                                </div>
                                <div class="col-5 details">
                                    <label for="email">Email:</label>
                                    <span data-value="email" name="email">'.$email.'</span><br>
                                    <h3 class="btn">Edit</h3>
                                </div>
                                <div class="col-5 details">
                                    <label for="userPass">Password:</label>
                                    <span data-value="userPass" name="userPass">'.$pass.'</span><br>
                                    <h3 class="btn">Edit</h3>
                                </div>
                                <div class="col-5 details">
                                    <label for="username">Username:</label>
                                    <span data-value="username" name="username">'.$row['username'].'</span><br>
                                    <h3 class="btn">Edit</h3>
                                </div>
                            </div>
                            <input type="hidden" name="email" value="' . $email . '">
                            ...
                            <button class="btn btn-light" name="updateInfoBtn" type="submit">Update Info</button>
                        </form>
                        <div>
                           <form action="editprofile.php" method="post">
                                <input type="hidden" name="email" value="' . $email . '">
                                ...
                                <input type="hidden" name="lname" value="' . $row['lname'] . '">
                                <input type="hidden" name="username" value="' . $row['username'] . '">
                                <button type="submit" class="btn btn-danger" name="deleteInfo">Delete this acount</button>
                           </form>
                        </div>
